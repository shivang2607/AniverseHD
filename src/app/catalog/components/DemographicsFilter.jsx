...
const DemographicsFilter = ({ currOpened, setCurrOpened }) => {
    const { demographics, toggleDemographic } = useAnimeSearchFilterStore();

    return (
      <div className="flex flex-col w-full bg-gray-800 p-2 rounded-lg shadow-lg relative border-[0.5px] border-slate-700">
        <div
          onClick={() => {
            currOpened === "demographics" ?  setCurrOpened(null) : setCurrOpened("demographics");
          }}
        ...
        >
          <h3 className="text-md font-semibold">Demographics</h3>
          <i
            className={`transform transition-transform ${
              currOpened == "demographics" ? "rotate-180" : "rotate-0"
            }`}
          >
        ...
        <div
          className={`absolute mt-11 z-10 w-full left-0 bg-gray-800 shadow-gray-600 shadow-md p-2 rounded-lg transition-all duration-700 ease-in-out overflow-hidden ${
            currOpened == "demographics"
              ? " opacity-100 visible "
              : " opacity-0 invisible"
          }`}
        >
